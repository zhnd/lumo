    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      {errorResult && (
        <div className="border-b px-6 py-2">
          <p className="mx-auto max-w-5xl text-xs text-destructive">{errorResult.message}</p>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-muted/40">
        <div className="mx-auto max-w-5xl p-6">
          ...
          {isError && <CardError message="Failed to load marketplace" onRetry={() => refetch()} />}
      ...
      <Sheet open={addSourceOpen} onOpenChange={setAddSourceOpen}>
        <SheetContent>
          <SheetHeader>
            ...
            <SheetDescription>Add or remove plugin marketplace sources.</SheetDescription>
          </SheetHeader>
          ...
          <div className="flex flex-col gap-4 px-4">
            <div className="flex gap-2">
              <Input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !isAddingMarketplace) onAddMarketplace();
                }}
            ...
            {addMarketplaceResult && !addMarketplaceResult.success && (
              <p className="text-xs text-destructive">{addMarketplaceResult.message}</p>
            )}
              ...
              {marketplaces.map((m) => (
                <div key={m.name} className="flex items-start gap-3 rounded-lg border p-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {m.ownerName && (
                      <p className="mt-1 text-[11px] text-muted-foreground">{m.ownerName}</p>
                    )}
                  </div>
                  <Button
                    variant="ghost"
